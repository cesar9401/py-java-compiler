...
export class OperationCheck {
	...
	uminusPY(left: Variable | undefined, line:number, column: number) : Variable | undefined {
		...
		const desc = `No es posible efectuar la operacion de 'negacion aritmetica' con una variable de tipo '${left.type}'.`;
		const error = new Error(line, column, '', TypeE.SEMANTICO, desc);
		this.sm.errors.push(error);
		return;
	}

	/* operaciones binarias java */
	binaryOperationJV(type: OperationType, left: Variable | undefined, right: Variable | undefined, line:number, column: number): Variable | undefined {
		if(!left || !right) {
			return undefined;
		}
	...
	/* negacion logica en java */
	notJV(left: Variable | undefined, line:number, column: number): Variable | undefined {
		if(!left) return;

		if(left.type === OperationType.BOOL) {
			return new Variable(OperationType.BOOL, undefined, " ");
		}
	...
	/* negacion aritmetica en java */
	uminusJV(left: Variable | undefined, line:number, column: number): Variable | undefined {
		if(!left) return;
